...
{
  {
    ...
    cbf_B.d1 = cbf_M->Timing.t[0];
    if ((cbf_DW.TimeStampA >= cbf_B.d1) && (cbf_DW.TimeStampB >= cbf_B.d1)) {
      cbf_B.Derivative1 = 0.0;
    } else {
      cbf_B.Derivative1 = cbf_DW.TimeStampA;
      lastU = &cbf_DW.LastUAtTimeA;
      if (cbf_DW.TimeStampA < cbf_DW.TimeStampB) {
        if (cbf_DW.TimeStampB < cbf_B.d1) {
          cbf_B.Derivative1 = cbf_DW.TimeStampB;
          lastU = &cbf_DW.LastUAtTimeB;
        }
      } else if (cbf_DW.TimeStampA >= cbf_B.d1) {
        cbf_B.Derivative1 = cbf_DW.TimeStampB;
        lastU = &cbf_DW.LastUAtTimeB;
      }

      cbf_B.Derivative1 = (cbf_B.MovingAverage1.MovingAverage - *lastU) /
        (cbf_B.d1 - cbf_B.Derivative1);
    }
    ...
    ParamGet_cbf_567.get_parameter(&cbf_B.cumRevIndex);

    // MATLABSystem: '<S16>/Get Parameter3'
    ParamGet_cbf_568.get_parameter(&cbf_B.csum);

    // MATLABSystem: '<S16>/Get Parameter4'
    ParamGet_cbf_569.get_parameter(&cbf_B.modValueRev);

    // MATLABSystem: '<S16>/Get Parameter5'
    ParamGet_cbf_570.get_parameter(&cbf_B.z);

    // MATLABSystem: '<S16>/Get Parameter7'
    ParamGet_cbf_595.get_parameter(&cbf_B.b_value);
    ...
    //   MATLABSystem: '<S16>/Get Parameter7'
    //
    cbf_B.cumRevIndex = (((cbf_B.cumRevIndex / cbf_B.b_value * cbf_B.Derivative1
      + cbf_B.d) + (cbf_B.In1_f.Data - cbf_B.cumRevIndex * cbf_B.Derivative1) *
                          (cbf_B.modValueRev + cbf_B.z)) + ((cbf_B.In1_j.Data -
      cbf_B.cumRevIndex * cbf_B.In1_m.Data) - cbf_B.csum) * (cbf_B.modValueRev *
      cbf_B.z)) * (cbf_B.b_value / cbf_B.cumRevIndex);
    ...
    if (cbf_P.Constant2_Value > cbf_P.Switch_Threshold) {
      ...
      if ((cbf_B.cumRevIndex <= cbf_B.In1.Data) || rtIsNaN(cbf_B.In1.Data)) {
        cbf_B.csum = cbf_B.cumRevIndex;
      } else {
        cbf_B.csum = cbf_B.In1.Data;
      }
    ...
    if (cbf_B.csum > cbf_P.minmax1550_UpperSat) {
      cbf_B.csum = cbf_P.minmax1550_UpperSat;
    } else if (cbf_B.csum < cbf_P.minmax1550_LowerSat) {
      cbf_B.csum = cbf_P.minmax1550_LowerSat;
    }

    // End of Saturate: '<Root>/min//max 1.5//-5.0'

    // BusAssignment: '<Root>/Bus Assignment1'
    cbf_B.BusAssignment1.Data = cbf_B.csum;
    ...
    cbf_B.BusAssignment2.Linear.Z = cbf_B.csum;
    ...
    cbf_B.BusAssignment2.Linear.X = cbf_B.d;
    ...
    // Derivative: '<Root>/Derivative'
    if ((cbf_DW.TimeStampA_h >= cbf_B.d1) && (cbf_DW.TimeStampB_e >= cbf_B.d1))
    {
      cbf_B.csum = 0.0;
    } else {
      cbf_B.Derivative1 = cbf_DW.TimeStampA_h;
      lastU = &cbf_DW.LastUAtTimeA_k;
      if (cbf_DW.TimeStampA_h < cbf_DW.TimeStampB_e) {
        if (cbf_DW.TimeStampB_e < cbf_B.d1) {
          cbf_B.Derivative1 = cbf_DW.TimeStampB_e;
          lastU = &cbf_DW.LastUAtTimeB_l;
        }
      } else if (cbf_DW.TimeStampA_h >= cbf_B.d1) {
        cbf_B.Derivative1 = cbf_DW.TimeStampB_e;
        lastU = &cbf_DW.LastUAtTimeB_l;
      }

      cbf_B.csum = (cbf_B.MovingAverage.MovingAverage - *lastU) / (cbf_B.d1 -
        cbf_B.Derivative1);
    }
    ...
    cbf_B.BusAssignment2 = cbf_P.Constant_Value_i;
    cbf_B.BusAssignment2.Linear.X = cbf_B.csum;
    ...
    Sub_cbf_552.getLatestMessage(&cbf_B.b_varargout_2_b);
