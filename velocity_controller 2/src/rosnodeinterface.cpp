...
// C/C++ source code generated on : Wed Oct 18 15:57:49 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"velocity_controller\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(velocity_controller_M, (NULL));
        velocity_controller_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(velocity_controller_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(velocity_controller_M)
                == (NULL)) || rtmGetStopRequested(velocity_controller_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(velocity_controller_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
